...
property_opts = [
    cfg.StrOpt('property_protection_file',
               help=_('The location of the property protection file.'
                      ...
                      'clictest-api service will not start.')),
...
class PropertyRules(object):
    ...
    def __init__(self, policy_enforcer=None):
        ...
        self.policies = []
        self.policy_enforcer = policy_enforcer or clictest.api.policy.Enforcer()
    ...
    def _add_policy_rules(self, property_exp, action, rule):
        ...
        [prop_a]
        create = clictest_creator
        then the corresponding policy rule would be:
        "prop_a:create": "rule:clictest_creator"
        where clictest_creator is defined in policy.json. For example:
        "clictest_creator": "role:admin or role:clictest_create_user"
        """
        rule = "rule:%s" % rule
