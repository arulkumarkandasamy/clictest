...
from clictest.common import auth
from clictest.common import exception
from clictest.common import utils
...
class BaseClient(object):
    ...
    def get_connect_kwargs(self):
        ...
        if self.use_ssl:
            if self.key_file is None:
                self.key_file = os.environ.get('CLICTEST_CLIENT_KEY_FILE')
            if self.cert_file is None:
                self.cert_file = os.environ.get('CLICTEST_CLIENT_CERT_FILE')
            if self.ca_file is None:
                self.ca_file = os.environ.get('CLICTEST_CLIENT_CA_FILE')
            ...
            if self.cert_file is not None and self.key_file is None:
                msg = _("You have selected to use SSL in connecting, "
                        ...
                        "CLICTEST_CLIENT_KEY_FILE environ variable")
            ...
            if self.key_file is not None and self.cert_file is None:
                msg = _("You have selected to use SSL in connecting, "
                        ...
                        "CLICTEST_CLIENT_CERT_FILE environ variable")
                raise exception.ClientConnectionError(msg)
    ...
    def _do_request(self, method, url, body, headers):
        ...
        any returned HTTP error status codes to OpenStack/Clictest exceptions
    ...
    def _seekable(self, body):
        ...
        #   cat /path/to/image | clictest add ...
