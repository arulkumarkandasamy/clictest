...
"""
Helper script for starting/stopping/reloading Clictest server programs.
...
ALL_COMMANDS = ['start', 'status', 'stop', 'shutdown', 'restart',
                'reload', 'force-reload']
ALL_SERVERS = ['api']
RELOAD_SERVERS = ['clictest-api']
GRACEFUL_SHUTDOWN_SERVERS = ['clictest-api']
...
def pid_files(server, pid_file):
    ...
    if pid_file:
        ...
        if os.path.exists('/var/run/clictest/%s.pid' % server):
            pid_files = ['/var/run/clictest/%s.pid' % server]
...
def add_command_parsers(subparsers):
    ...
    parser.set_defaults(servers=['clictest-' + s for s in ALL_SERVERS])
